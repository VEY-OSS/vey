        ...
        match value {
            Yaml::Hash(map) => {
                ...
                vey_yaml::foreach_kv(map, |k, v| match vey_yaml::key::normalize(k).as_str() {
                    "address" | "addr" => {
                        let addr = vey_yaml::value::as_env_sockaddr(v)?;
                    ...
                    "shared_key" => {
                        let key = vey_yaml::value::as_string(v)?;
                    ...
                    "password" => {
                        let pass = vey_yaml::value::as_string(v)?;
                        config.set_password(pass);
                        Ok(())
                    }
                    "hostname" => {
                        let hostname = vey_yaml::value::as_string(v)?;
                    ...
                    "tls" | "tls_client" => {
                        let tls_config =
                            vey_yaml::value::as_to_one_openssl_tls_client_config_builder(
                                v, lookup_dir,
                            )
                            .context(format!(
