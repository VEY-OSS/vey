...
async fn to_chunked_counts_data_payload() {
    ...
    assert_eq!(xfer.received_size(), payload.len() as u64);
    assert_eq!(&write_buf, b"9\r\ntest\nbody\r\n0\r\n\r\n");
}

#[tokio::test]
async fn to_chunked_counts_received_payload_on_write_error() {
    let payload = b"test\nbody";
    let (mut client, server) = handshake().await;
    let accepted = spawn_server(server);
    ...
    let resp = resp_fut.await.unwrap();
    let mut recv = resp.into_body();
    // the chunk head is written, then the payload write fails
    let mut writer = tokio_test::io::Builder::new()
        .write(b"9\r\n")
        .write_error(std::io::Error::other("write failed"))
        .build();
    let mut xfer = H2StreamToChunkedTransfer::new(&mut recv, &mut writer, 1024);
    assert!((&mut xfer).await.is_err());
