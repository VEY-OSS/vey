...
#[derive(Debug, Error)]
pub enum H2ToH1RespmodAdaptationError {
    #[error("write to icap server failed: {0:?}")]
    IcapServerWriteFailed(io::Error),
    #[error("read from icap server failed: {0:?}")]
    IcapServerReadFailed(io::Error),
...
impl From<H2PreviewError> for H2ToH1RespmodAdaptationError {
    fn from(value: H2PreviewError) -> Self {
        match value {
            ...
            H2PreviewError::ReadIdle => H2ToH1RespmodAdaptationError::HttpUpstreamReadIdle,
            H2PreviewError::IdleForceQuit(r) => H2ToH1RespmodAdaptationError::IdleForceQuit(r),
        }
    }
}
