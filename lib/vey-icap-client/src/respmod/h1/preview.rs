...
impl<I: IdleCheck> HttpResponseAdapter<I> {
        ...
        UW: AsyncWrite + Unpin,
    {
        ...
        macro_rules! record_progress {
            () => {{
                state.ups_rsp_body_size = Some(preview_len + body_copy.reader().body_size());
                state.clt_rsp_body_size = Some(preview_len + body_copy.copied_size());
            }};
        }
        ...
        UW: AsyncWrite + Unpin,
    {
        ...
        macro_rules! record_progress {
            () => {{
                ...
                // the chunked body is copied as on-wire bytes, so the size sent
                // to the client is approximated by what has been read
                state.clt_rsp_body_size = Some(chunked_transfer.written_body_size());
            }};
        }
