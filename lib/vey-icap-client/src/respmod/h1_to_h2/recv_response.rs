...
impl<I: IdleCheck> H1ToH2ResponseAdapter<I> {
    ...
    pub(super) async fn handle_original_http_response_with_body<UR, CW>(
        ...
        ups_body_io: &mut UR,
        preview_buf: Option<Vec<u8>>,
        left_chunk_size: Option<u64>,
        clt_send_response: &mut CW,
    ) -> Result<RespmodAdaptationEndState, H1ToH2RespmodAdaptationError>
    where
        ...
        CW: H2SendResponseToClient,
    {
        ...
        let preview_len = preview_buf.as_ref().map(|b| b.len() as u64).unwrap_or(0);
        if let Some(buf) = preview_buf
            && !buf.is_empty()
        {
            clt_send_stream
                .send_data(Bytes::from(buf), false)
                .map_err(H1ToH2RespmodAdaptationError::HttpClientSendDataFailed)?;
        }

        match body_type {
            HttpBodyType::Chunked => {
                let mut body_reader = if let Some(left) = left_chunk_size {
                    HttpBodyDecodeReader::new_chunked_after_preview(
                        ups_body_io,
                        self.http_body_line_max_size,
                        left,
                    )
                } else {
                    HttpBodyDecodeReader::new_chunked(ups_body_io, self.http_body_line_max_size)
                };
                Self::send_decoded_to_h2(
                    &self.idle_checker,
                    &self.copy_config,
            ...
            HttpBodyType::ContentLength(n) => {
                let mut body_reader = HttpBodyDecodeReader::new_fixed_length(ups_body_io, n);
                Self::send_decoded_to_h2(
                    &self.idle_checker,
                    &self.copy_config,
                    self.http_trailer_max_size,
            ...
            HttpBodyType::ReadUntilEnd => {
                let mut body_reader = HttpBodyDecodeReader::new_read_until_end(ups_body_io);
                Self::send_decoded_to_h2(
                    &self.idle_checker,
                    &self.copy_config,
                    self.http_trailer_max_size,
        ...
        R: AsyncBufRead + Unpin,
    {
        let mut body_transfer =
            H2BodyEncodeTransfer::new(body_reader, clt_send_stream, copy_config);
        ...
        let copied = preview_len + body_transfer.copied_size();
        state.ups_rsp_body_size = Some(copied);
        drop(body_transfer);
