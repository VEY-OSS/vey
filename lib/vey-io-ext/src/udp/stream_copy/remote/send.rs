...
pub struct LimitedUdpCopyRemoteSend<T> {
    inner: T,
    delay: Pin<Box<Sleep>>,
    started: Instant,
...
impl<T: UdpCopyRemoteSend> LimitedUdpCopyRemoteSend<T> {
    pub fn unlimited(inner: T, stats: ArcLimitedSendStats) -> Self {
        LimitedUdpCopyRemoteSend {
            inner,
...
impl<T> UdpCopyRemoteSend for LimitedUdpCopyRemoteSend<T>
where
    T: UdpCopyRemoteSend + Unpin,
{
    #[cfg(feature = "log")]
    fn error_logger(&self) -> Option<&Logger> {
        self.inner.error_logger()
    }
    ...
    fn poll_send_many_packets(
        ...
        if self.limit.is_set() {
            ...
            let count = ready!(self.inner.poll_send_many_packets(cx, packets))?;
            self.stats.add_send_packets(count);
            self.stats
    ...
    fn poll_send_many_bytes(
        ...
        if self.limit.is_set() {
            ...
            let count = ready!(self.inner.poll_send_many_bytes(cx, packets))?;
            self.stats.add_send_packets(count);
            self.stats
                .add_send_bytes(packets.iter().take(count).map(|h| h.len()).sum());
            Poll::Ready(Ok(count))
        }
    }
}
