...
impl RedisClientConfigBuilder {
    pub fn set_by_yaml_kv(
        ...
        match vey_yaml::key::normalize(k).as_str() {
            "addr" | "address" => {
                let addr = vey_yaml::value::as_upstream_addr(v, crate::REDIS_DEFAULT_PORT)
                    .context(format!("invalid upstream address value for key {k}"))?;
                self.set_addr(addr);
                Ok(())
            }
            "tls" | "tls_client" => {
                let tls = vey_yaml::value::as_rustls_client_config_builder(v, lookup_dir).context(
            ...
            "db" => {
                let db =
                    vey_yaml::value::as_i64(v).context(format!("invalid int value for key {k}"))?;
                self.set_db(db);
                Ok(())
            }
            "username" => {
                let username = vey_yaml::value::as_string(v)?;
                self.set_username(username);
                Ok(())
            }
            "password" => {
                let password = vey_yaml::value::as_string(v)?;
