...
use super::{ResolveError, ResolveServerError, ResolvedRecord};
...
pub struct ResolverQuerySnapshot {
    pub query_total: u64,
    pub query_cached: u64,
    pub query_driver: u64,
    pub query_trashed: u64,
...
impl ResolverQueryStats {
    fn snapshot(&self) -> ResolverQuerySnapshot {
        ResolverQuerySnapshot {
            ...
            query_cached: self.query_cached.load(Ordering::Relaxed),
            query_driver: self.query_driver.load(Ordering::Relaxed),
            query_trashed: self.query_trashed.load(Ordering::Relaxed),
            query_timeout: self.query_timeout.load(Ordering::Relaxed),
    ...
    pub(crate) fn add_error(&self, e: &ResolveError) {
        match e {
            ...
            ResolveError::RequestTimeout => self.add_query_timeout(),
            _ => {} // others are all setup errors or errors generated by callers
        }
    }
}
