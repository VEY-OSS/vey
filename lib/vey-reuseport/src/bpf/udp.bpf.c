...
static long select_latest(struct bpf_map *map, const void *key, void *value, void *ctx)
{
...
int udp_select_reuseport(struct sk_reuseport_md *ctx)
{
	...
	if (sock_id) {
		if (bpf_sk_select_reuseport(ctx, &socket_map, sock_id, 0) == 0)
			return SK_PASS;
		/*
		 * Socket is gone (worker removed). Delete the stale entry
		 * and fall through to CPU-based selection.
		 */
	...
	struct proc_info_result latest = {};
	bpf_for_each_map_elem(&proc_map, select_latest, &latest, 0);
	if (!latest.k || !latest.v) {
		return SK_PASS;
	}
	selected.pid = latest.k->pid;
	selected.generation = latest.k->generation;
	selected.worker = random % latest.v->count;
	if (bpf_sk_select_reuseport(ctx, &socket_map, &selected, 0) == 0) {
		bpf_map_update_elem(&conn_track, &hash, &selected, 0);
		return SK_PASS;
	} else {
		// Mark the selected pid+generation as invalid
		__sync_fetch_and_add(&latest.v->invalid, 1);
	}
