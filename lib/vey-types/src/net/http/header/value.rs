...
impl FromStr for H1HeaderValue {
...
impl From<H1HeaderValue> for HeaderValue {
    fn from(value: H1HeaderValue) -> Self {
        value.into_inner()
    }
}

impl AsRef<HeaderValue> for H1HeaderValue {
...
mod tests {
    ...
    fn http_header_value_operations() {
        // basic constructors
        let static_value = H1HeaderValue::from_static("static_value");
        ...
        let unsafe_string_value =
            unsafe { H1HeaderValue::from_string_unchecked("unsafe_string".to_string()) };
        ...
        let unsafe_buf_value =
            unsafe { H1HeaderValue::from_buf_unchecked(vec![b't', b'e', b's', b't']) };
        assert_eq!(unsafe_buf_value.to_str(), "test");
        assert_eq!(unsafe_buf_value.as_bytes(), b"test");

        // parsing from string
        let parsed_value: H1HeaderValue = "parsed-value".parse().unwrap();
        assert_eq!(parsed_value.to_str(), "parsed-value");

        // invalid parsing
        assert!("invalid\nvalue".parse::<H1HeaderValue>().is_err());

        // original name functionality
        let mut value_with_name = H1HeaderValue::from_static("test");
        ...
        // value mutation
        let mut mutable_value = H1HeaderValue::from_static("initial");
        ...
        // accessors
        let accessor_value = H1HeaderValue::from_static("access_test");
        assert_eq!(
        ...
        let non_ascii_value = unsafe { H1HeaderValue::from_string_unchecked("café".to_string()) };
        assert_eq!(non_ascii_value.to_str(), "café");

        // into_inner
        let into_inner_value = H1HeaderValue::from_static("inner_test");
        let inner: HeaderValue = into_inner_value.into_inner();
        assert_eq!(inner, HeaderValue::from_static("inner_test"));

        // conversion traits
        let conversion_value = H1HeaderValue::from_static("conversion_test");
        let header_value: HeaderValue = conversion_value.into();
        assert_eq!(header_value, HeaderValue::from_static("conversion_test"));

        let as_ref_value = H1HeaderValue::from_static("as_ref_test");
        ...
        let normal_value = H1HeaderValue::from_static("application/json");
        normal_value.write_to_buf(&header_name, &mut buf);
        assert_eq!(buf, b"content-type: application/json\r\n");

        // with original name
        buf.clear();
        let mut value_with_orig = H1HeaderValue::from_static("application/json");
