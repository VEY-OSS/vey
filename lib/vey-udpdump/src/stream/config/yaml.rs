        ...
        match value {
            Yaml::Hash(map) => {
                ...
                vey_yaml::foreach_kv(map, |k, v| match vey_yaml::key::normalize(k).as_str() {
                    "peer" => {
                        config.peer = vey_yaml::value::as_env_sockaddr(v)?;
                        Ok(())
                    }
                    "socket_buffer" => {
                        config.buffer = vey_yaml::value::as_socket_buffer_config(v)
                            .context(format!("invalid socket buffer config value for key {k}"))?;
                        Ok(())
                    }
                    "misc_opts" => {
                        config.opts = vey_yaml::value::as_udp_misc_sock_opts(v)
                            .context(format!("invalid udp misc socket option value for key {k}"))?;
                        Ok(())
                    }
                    "packet_size" => {
                        config.packet_size = vey_yaml::value::as_usize(v)?;
                        Ok(())
                    }
                    "client_side" => {
                        config.client_side = vey_yaml::value::as_bool(v)?;
                        Ok(())
                    }
            ...
            Yaml::String(_) => {
                let config = StreamDumpConfig {
                    peer: vey_yaml::value::as_env_sockaddr(value)?,
                    ..Default::default()
                };
                Ok(config)
            }
