...
TEST_NAME="vey-proxy-ci"
. "${SCRIPTS_DIR}/enter.sh"

# build
cargo build --features jemalloc,ebpf,lua54 -p vey-proxy -p vey-proxy-ctl -p vey-proxy-ftp -p vey-mkcert -p vey-dcgen -p vey-iploc -p vey-statsd
...
cargo test -p vey-proxy -p vey-proxy-ctl -p vey-proxy-ftp

RUN_DIR="${SCRIPTS_DIR}/vey-proxy"
. "${RUN_DIR}/run.sh"
