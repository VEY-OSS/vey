...
resolver_query_verify()
{
	...
	result=$("${PROJECT_DIR}"/target/debug/vey-proxy-ctl -G ${TEST_NAME} -p $PROXY_PID resolver ${resolver} query ${domain})
	if [ $expect != $result ]
	then
...
do
	...
	resolver_query_verify ${resolver} vey-proxy.local 127.0.0.1
