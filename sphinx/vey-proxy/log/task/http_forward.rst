...
body is sent upstream. If this hop has started, the value is the decoded
payload sent so far, including when the task later fails. Omitted if the
upstream request body has not started.

An HTTP/1.1 chunked body is forwarded as on-wire bytes, so the decoded payload
is not counted as it is written. When such a write does not finish, a lower
bound is reported instead: the payload read on the way in, less everything still
buffered when the task failed, since all of those bytes could be payload. The
gap is at most one copy buffer.
...
upstream response body has not started.
