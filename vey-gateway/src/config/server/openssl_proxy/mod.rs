    ...
    fn set(&mut self, k: &str, v: &Yaml) -> anyhow::Result<()> {
        match vey_yaml::key::normalize(k).as_str() {
            ...
            "client_hello_max_size" => {
                self.client_hello_max_size = vey_yaml::humanize::as_u32(v)
                    .context(format!("invalid humanize u32 value for key {k}"))?;
                Ok(())
            }
            "accept_timeout" | "handshake_timeout" | "negotiation_timeout" => {
                self.accept_timeout = vey_yaml::humanize::as_duration(v)
                    .context(format!("invalid humanize duration value for key {k}"))?;
                Ok(())
            }
            "virtual_hosts" | "hosts" => {
                self.hosts = vey_yaml::value::as_host_matched_obj(v, self.position.as_ref())?;
                Ok(())
            }
            ...
            #[cfg(feature = "openssl-async-job")]
            "tls_no_async_mode" => {
                self.tls_no_async_mode = vey_yaml::value::as_bool(v)?;
                Ok(())
            }
