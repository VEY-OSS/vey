...
You can run `cargo run --bin vey-iploc -- -c vey-iploc/examples/basic/ -G port2888 -vv` to start it.
...
- Via command line options
  ...
  You can set the systemd instance name to *port<port>*, so when you run `systemctl start vey-iploc@port<port>`,
...
- Via environment variables
  ...
  You can add this environment variable to `/etc/vey-iploc/<instance name>/env` file to use this with
  systemd managed vey-iploc service.
...
vey-iploc can only load databases in VEY native CSV format, which can be generated by
...
- [Maxmind](https://www.maxmind.com/en/geoip-databases)
    ...
    3. Convert by using [geoip-dump](../scripts/geoip-dump) scripts
       ...
       ```shell
       python3 geoip_dump_country.py --maxmind -i GeoLite2-Country.mmdb -o vey-country.csv.gz
       python3 geoip_dump_asn.py --maxmond -i GeoLite2-ASN.mmdb -o vey-asn.csv.gz
       ```
...
- [IPinfo](https://ipinfo.io/)
    ...
    3. Convert by using [geoip-dump](../scripts/geoip-dump) scripts
       ...
       ```shell
       python3 geoip_dump_country.py --ipinfo -i ipinfo_lite.mmdb -o vey-country.csv.gz
       python3 geoip_dump_asn.py --ipinfo -i ipinfo_lite.mmdb -o vey-asn.csv.gz
       ```
...
- [IPFire](https://www.ipfire.org/)
    ...
    4. Convert by using [geoip-dump](../scripts/geoip-dump) scripts
       ...
       ```shell
       python3 geoip_dump_country.py --ipfire -i location.db -o vey-country.csv.gz
       python3 geoip_dump_asn.py --ipfire -i location.db -o vey-asn.csv.gz
       ```

## Command line options

Just run `vey-iploc -h` to see all supported command line options.
