# Use Intel QAT Engine with vey-keyless
...
## Example vey-keyless config

Assume that the config directory is `/etc/vey-keyless/test`, and the openssl.cnf file path is `/etc/ssl/qat.cnf`

then you need to have

`/etc/vey-keyless/test/env` contains

```text
OPENSSL_CONF=/etc/ssl/qat.cnf
```

`/etc/vey-keyless/test/main.yaml` can be written as:
