...
3. 执行`systemctl restart vey-proxy@<daemon_group>`，启动新进程，并通知老进程离线
...
vey-proxy采用模块化方式进行功能设计，主要包含以下功能模块：
...
server:
  - name: http-in
    type: http_proxy
    escaper: chain
    username_params: # 所有Key都会被加入到Egress Context中
      required_keys: host
...
escaper:
  - name: comply_http
    type: comply_context # 从Egress Context中提取值，并为下一跳出口设置动态地址
    next: proxy_http
    use_egress_upstream:
      default_port: 8080
      host_key: host
  ...
  - name: proxy_http
    type: proxy_http # 使用comply_http出口设置的动态代理地址
    proxy_addr: 127.0.0.1:3128 # 默认地址
```
...
实际下一跳代理地址为`proxy1.example.net`，并且启用使用值`1234`启用粘性解析。
...
有些场景可能要求使用国密协议访问，很多客户端不支持国密协议，可使用vey-proxy进行协议转换：
...
注意该功能需搭配tls cert generator使用，参考实现为[vey-dcgen](/vey-dcgen)
，示例配置参考[vey-dcgen simple conf](/vey-dcgen/examples/simple)。
