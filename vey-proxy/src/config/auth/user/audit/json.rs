            ...
            for (k, v) in map {
                match vey_json::key::normalize(k).as_str() {
                    "enable_protocol_inspection" => {
                        self.enable_protocol_inspection = vey_json::value::as_bool(v)
                            .context(format!("invalid bool value for key {k}"))?;
                    }
                    "prohibit_unknown_protocol" => {
                        self.prohibit_unknown_protocol = vey_json::value::as_bool(v)
                            .context(format!("invalid bool value for key {k}"))?;
                    }
                    "prohibit_timeout_protocol" => {
                        self.prohibit_timeout_protocol = vey_json::value::as_bool(v)
                            .context(format!("invalid bool value for key {k}"))?;
                    }
                    "task_audit_ratio" | "application_audit_ratio" => {
                        let ratio = vey_json::value::as_random_ratio(v)
