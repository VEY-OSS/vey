...
impl UserConfig {
    pub(crate) fn parse_yaml(
        ...
        vey_yaml::foreach_kv(map, |k, v| config.set_yaml(k, v, position))?;
        config.check()?;
        Ok(config)
    }
    ...
    fn set_yaml(
        ...
        match vey_yaml::key::normalize(k).as_str() {
            "name" => {
                let name =
                    vey_yaml::value::as_string(v).context(format!("invalid value for key {k}"))?;
                self.name = name.into();
                Ok(())
            }
            "token" => {
            ...
            "match_by_facts" => {
                self.match_by_facts =
                    vey_yaml::value::as_list(v, vey_yaml::value::as_facts_match_value)
                        .context(format!("invalid FactsMatchValue list for key {k}"))?;
                Ok(())
            }
            "expire" => {
                let expire_datetime = vey_yaml::value::as_rfc3339_datetime(v)
            ...
            "tcp_connect" => {
                let config = vey_yaml::value::as_tcp_connect_config(v)
            ...
            "tcp_all_upload_speed_limit" => {
                let limit = vey_yaml::value::as_global_stream_speed_limit(v).context(format!(
            ...
            "udp_all_upload_speed_limit" => {
                let limit = vey_yaml::value::as_global_datagram_speed_limit(v).context(format!(
            ...
            "http_upstream_keepalive" => {
                self.http_upstream_keepalive = vey_yaml::value::as_http_keepalive_config(v)
            ...
            "connection_rate_limit" => {
                let quota = vey_yaml::value::as_rate_limit_quota(v)
            ...
            "request_max_alive" | "request_alive_max" => {
                self.request_alive_max = vey_yaml::value::as_usize(v)
            ...
            "proxy_request_filter" => {
                let filter = vey_yaml::value::acl::as_proxy_request_rule(v)
                    .context(format!("invalid proxy request acl rule for key {k}"))?;
                self.proxy_request_filter = Some(filter);
                Ok(())
            }
            "dst_host_filter_set" => {
                let builder = vey_yaml::value::acl_set::as_dst_host_rule_set_builder(v)
            ...
            "log_uri_max_chars" | "uri_log_max_chars" => {
                let max_chars = vey_yaml::value::as_usize(v)
                    .context(format!("invalid usize value for key {k}"))?;
            ...
            "socks_use_udp_associate" => {
                self.socks_use_udp_associate = vey_yaml::value::as_bool(v)
            ...
            "egress_path_value_map" => {
                let value_map =
                    vey_yaml::value::as_hashmap(v, vey_yaml::value::as_metric_node_name, |v| {
