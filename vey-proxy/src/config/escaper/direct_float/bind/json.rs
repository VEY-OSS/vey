...
impl DirectFloatBindIp {
    pub(crate) fn parse_json(
        ...
        match value {
            Value::Object(map) => {
                let ip_v = vey_json::map_get_required(map, CONFIG_KEY_IP)?;
                let ip = vey_json::value::as_ipaddr(ip_v)
                ...
                for (k, v) in map {
                    match vey_json::key::normalize(k).as_str() {
                        CONFIG_KEY_IP => {}
                        CONFIG_KEY_ID => {
                            let id = vey_json::value::as_string(v)?;
                            bind.id = Some(id);
                        }
                        CONFIG_KEY_EXPIRE => {
                            let datetime_expire = vey_json::value::as_rfc3339_datetime(v)?;
            ...
            Value::String(_) => {
                let ip = vey_json::value::as_ipaddr(value)
