...
use vey_types::net::{
    HappyEyeballsConfig, Host, SocketBufferConfig, SocksAuth, TcpKeepAliveConfig, TcpMiscSockOpts,
    UdpMiscSockOpts, WeightedUpstreamAddr,
};
        ...
        match vey_yaml::key::normalize(k).as_str() {
            ...
            "proxy_addr" => {
                self.proxy_nodes = vey_yaml::value::as_list(v, |v| {
                    vey_yaml::value::as_weighted_upstream_addr(v, 1080)
                })
                .context(format!(
            ...
            "end_on_control_closed" => {
                self.end_on_control_closed = vey_yaml::value::as_bool(v)?;
                Ok(())
            }
