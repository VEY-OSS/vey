...
const SERVER_CONFIG_TYPE: &str = "HttpExpose";

/// collection of timeout config
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct HttpExposeServerTimeoutConfig {
...
impl Default for HttpExposeServerTimeoutConfig {
    fn default() -> Self {
        HttpExposeServerTimeoutConfig {
...
pub(crate) struct HttpExposeServerConfig {
    name: NodeName,
    ...
    pub(crate) timeout: HttpExposeServerTimeoutConfig,
...
impl HttpExposeServerConfig {
    fn new(position: Option<YamlDocPosition>) -> Self {
        HttpExposeServerConfig {
            ...
            timeout: HttpExposeServerTimeoutConfig::default(),
    ...
    pub(crate) fn parse(
        ...
        let mut server = HttpExposeServerConfig::new(position);
...
impl ServerConfig for HttpExposeServerConfig {
    ...
    fn diff_action(&self, new: &AnyServerConfig) -> ServerConfigDiffAction {
        let AnyServerConfig::HttpExpose(new) = new else {
...
"#,
        ...
        assert!(HttpExposeServerConfig::parse(map, None).is_err());
    }

    #[test]
    fn parse_site_group_only() {
        let yaml = YamlLoader::load_from_str(
            r#"
name: local_in
type: http_expose
...
"#,
        ...
        let server = HttpExposeServerConfig::parse(map, None).unwrap();
