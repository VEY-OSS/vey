...
use vey_types::net::{DomainName, Host, HttpForwardCapability, UpstreamAddr, WeightedUpstreamAddr};
...
impl ProxySocks5Escaper {
    ...
    pub(super) fn prepare_initial(config: ProxySocks5EscaperConfig) -> anyhow::Result<ArcEscaper> {
        ...
        ProxySocks5Escaper::new_obj(config, stats, peer_health_table)
    }
...
impl Escaper for ProxySocks5Escaper {
    ...
    async fn tcp_setup_connection(
        ...
        self.socks5_new_tcp_connection(task_conf, egress_notes, task_notes, task_stats)
            .await
    }
    ...
    async fn udp_setup_connection(
        ...
        egress_notes.escaper.clone_from(&self.config.name);
        self.udp_new_connection(task_conf, egress_notes, task_notes, task_stats)
            .await
    }
    ...
    async fn udp_setup_relay(
        ...
        egress_notes.escaper.clone_from(&self.config.name);
        self.udp_setup_relay(task_conf, egress_notes, task_notes, task_stats)
            .await
    }
...
impl EscaperInternal for ProxySocks5Escaper {
    ...
    fn _reload(
        ...
        ProxySocks5Escaper::prepare_reload(config, stats, peer_health_table)
    }
    ...
    async fn _nested_tcp_connect(
        ...
        self.stats.interface.add_tcp_connect_attempted();
        egress_notes.escaper.clone_from(&self.config.name);
        self.socks5_nested_tcp_connect(task_conf, egress_notes, task_notes)
            .await
    }
