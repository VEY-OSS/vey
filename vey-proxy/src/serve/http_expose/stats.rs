...
pub(crate) struct HttpExposeServerStats {
    name: NodeName,
    id: StatId,
...
impl HttpExposeServerStats {
    pub(super) fn new(name: &NodeName) -> Self {
        HttpExposeServerStats {
            name: name.clone(),
...
pub(super) struct HttpForwardTaskAliveGuard(Arc<HttpExposeServerStats>);
...
pub(super) struct HttpUntrustedTaskAliveGuard(Arc<HttpExposeServerStats>);
...
impl ServerStats for HttpExposeServerStats {
    #[inline]
