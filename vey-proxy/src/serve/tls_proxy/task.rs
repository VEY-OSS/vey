...
impl TlsProxyTask {
    ...
    async fn relay<S, R, W>(&mut self, clt_stream: S, ups_r: R, ups_w: W) -> ServerTaskResult<()>
    where
        S: AsyncStream + 'static,
        S::R: AsyncRead + Send + Sync + Unpin + 'static,
        S::W: AsyncWrite + Send + Sync + Unpin + 'static,
    ...
    fn split_clt<S>(
        ...
        LimitedReader<impl AsyncRead + use<S>>,
        LimitedWriter<impl AsyncWrite + use<S>>,
    )
    where
        S: AsyncStream + 'static,
        S::R: AsyncRead,
        S::W: AsyncWrite,
    {
        ...
        let mut limit_config = self.ctx.server_config.tcp_sock_speed_limit;
        limit_config = self
            .host
            .site()
